...
static const uint8_t delays[]={45,75,135,255}; //*0.1ms
...
static __pdata struct Bmp180 b;

static __pdata uint16_t jiffers0, jiffers1;
static __bit busy=0; //act as a lock of bmp180

static void calib_t(void){
	static __pdata int32_t temp=((uint16_t)b.buf[0]<<8)+b.buf[1];
	temp=((temp-b.ac6)*b.ac5) >> 15; //X1
	b.b5=temp + (b.mc<<11)/(temp+b.md);
//	return (b.b5+8)>>4;
}

static int32_t calib_p(void){
	static __pdata int32_t b6=b.b5-4000, b3, x, p=((uint32_t)b.buf[0]<<16)+((uint16_t)b.buf[1]<<8)+b.buf[2];
	static __pdata uint32_t b4,b7;
	x=((b.b2*((b6*b6)>>12))>>11)+((b.ac2*b6)>>11);
...
PT_V(bmp180_init){
	...
	//calib params are big endian, have to be swapped to little endian for sdcc
		for(i=0;i<CALIBLEN;++i,++i){
			temp=b.cbuf[i];
			b.cbuf[i]=b.cbuf[i+1];
			b.cbuf[i+1]=temp;
		}
...
PT(bmp180_measure_p, int8_t oss,__pdata int32_t * p){
	PT_B
		PT_LOCK(busy);
		b.oss=oss&0x03;
		b.buf[0]=CTR;
